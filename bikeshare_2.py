...
def get_filters():
    ...
    while city_name.lower() not in CITY_DATA:
        city_name = input("\nWhat is the name of the city you want to analyze data? (Input either chicago, new york city, washington)\n")
        if city_name.lower() in CITY_DATA:
            ...
            print("Sorry we were not able to get the name of the city to analyze data. Input either chicago, new york city or washington.\n")

    # TO DO: get user input for month (all, january, february, ... , june)
    month_name = ''
    while month_name.lower() not in MONTH_DATA:
        month_name = input("\nWhat is the name of the month to filter data? (Input either 'all' to apply no month filter or january, february, ... , june)\n")
...
def display_raw_data(df):
    """Displays raw data on request.
    Args:
        (DataFrame) df - Pandas DF containing city data filtered by month & day
    """
    print(df.head())
    next = 0
    while True:
